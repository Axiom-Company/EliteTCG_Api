...
import { sendPromotionConfirmation } from '../utils/email.js';
...
router.post('/purchase', authenticateCustomer, requireSeller, async (req, res) => {
  try {
    ...
    const promoNotifyUrl = process.env.PAYFAST_PROMO_NOTIFY_URL
      || 'https://api.elitetcg.co.za/api/marketplace/promotions/notify';
    ...
    if (paymentStatus === 'COMPLETE') {
      ...
      if (listing) {
        const { data: seller } = await supabaseAdmin
          ...
          .eq('customer_id', listing.seller_id)
          .single();

        if (seller) {
          const sellerEmail = seller.contact_email || seller.payfast_email;
          if (sellerEmail) {
            sendPromotionConfirmation(sellerEmail, seller.display_name, listing.title, promotion.tier, promotion.expires_at);
          }
        }
      }
