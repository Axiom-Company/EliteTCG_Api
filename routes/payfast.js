...
import { sendOrderConfirmation, sendSellerSaleNotification, sendSellerPayoutNotification, sendDeliveryConfirmedToSeller } from '../utils/email.js';
  ...
  try {
    ...
    if (paymentStatus === 'COMPLETE') {
      ...
      sendOrderConfirmation(
        ...
        [{ name: order.listing_title || 'Marketplace Item', quantity: order.quantity, unit_price_zar: order.unit_price }]
      );

      // Notify seller about the sale + payout
      const { data: seller } = await supabaseAdmin
        .from('seller_profiles')
        .select('display_name, contact_email, payfast_email')
        .eq('customer_id', order.seller_id)
        .single();

      if (seller) {
        const sellerEmail = seller.contact_email || seller.payfast_email;
        if (sellerEmail) {
          sendSellerSaleNotification(sellerEmail, seller.display_name, order.order_number, order.listing_title || 'Marketplace Item', order.quantity, order.seller_amount);
          sendSellerPayoutNotification(sellerEmail, seller.display_name, order.order_number, order.seller_amount);
        }
      }
  ...
  try {
    ...
    if (fullOrder) {
      const { data: seller } = await supabaseAdmin
        .from('seller_profiles')
        .select('display_name, contact_email, payfast_email')
        .eq('customer_id', fullOrder.seller_id)
        .single();

      if (seller) {
        const sellerEmail = seller.contact_email || seller.payfast_email;
        if (sellerEmail) {
          sendDeliveryConfirmedToSeller(sellerEmail, seller.display_name, fullOrder.order_number);
        }
      }
    }
